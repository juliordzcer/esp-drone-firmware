...
// --- CONFIGURACIÓN DEL ACCESS POINT DEL DRON ---
#ifndef DRONE_AP_SSID_BASE
#define DRONE_AP_SSID_BASE "ESP-DRONE"
#endif

#ifndef DRONE_AP_PASSWORD
#define DRONE_AP_PASSWORD "espdrone123"
#endif
...
static void espnow_ctrl_data_cb(espnow_attribute_t initiator_attribute, espnow_attribute_t responder_attribute, uint32_t status1, int status2, int lx_value, int ly_value, int rx_value, int ry_value, int channel_one_value, int channel_two_value)
{
    UDPPacket inPacket;
    inPacket.size = 7;
    inPacket.data[0] = 'n';
...
static void udp_server_rx_task(void *pvParameters)
{
    ...
    while (true) {
        if (!isUDPInit) { vTaskDelay(20); continue; }
...
void wifiInit(void)
{
    ...
    ESP_ERROR_CHECK(esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &wifi_event_handler, (void*)wifi_event_group, NULL));
    ESP_ERROR_CHECK(esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &wifi_event_handler, (void*)wifi_event_group, NULL));
    
    // --- INTENTAR MODO ESTACIÓN (Cliente) ---
    DEBUG_PRINT_LOCAL("Attempting to connect to home Wi-Fi: %s", HOME_WIFI_SSID);
    current_netif = esp_netif_create_default_wifi_sta();

    // ==================================================================
    // === INICIO: CÓDIGO AÑADIDO PARA IP FIJA ==========================
    // ==================================================================
    
    // 1. Detener el cliente DHCP para poder asignar una IP estática
    ESP_ERROR_CHECK(esp_netif_dhcpc_stop(current_netif));

    // 2. Definir la información de la IP estática
    esp_netif_ip_info_t ip_info;
    
    // !!! IMPORTANTE: Cambia estos valores para que coincidan con tu red local !!!
    IP4_ADDR(&ip_info.ip, 192, 168, 1, 100);       // IP estática para el dron
    IP4_ADDR(&ip_info.gw, 192, 168, 1, 254);         // IP de tu router (puerta de enlace)
    IP4_ADDR(&ip_info.netmask, 255, 255, 255, 0);  // Máscara de subred

    // 3. Aplicar la configuración de IP estática a la interfaz Wi-Fi STA
    ESP_ERROR_CHECK(esp_netif_set_ip_info(current_netif, &ip_info));
    
    // ==================================================================
    // === FIN: CÓDIGO AÑADIDO PARA IP FIJA =============================
    // ==================================================================
    ...
    if (bits & CONNECTED_BIT) {
        ...
        current_wifi_channel = WIFI_CHANNEL_DEFAULT;

        current_netif = esp_netif_create_default_wifi_ap();
        ...
        ESP_ERROR_CHECK(esp_netif_set_ip_info(current_netif, &ip_info_ap));
    ...
    xTaskCreate(udp_server_tx_task, "udp_tx", 4096, NULL, 5, NULL); // Nombres y tamaños de stack genéricos
